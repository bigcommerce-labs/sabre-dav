...
class Server {
     ...
     *
     * This is a simplified version of getPathProperties.
    ...
    public function getPropertiesForPath($path, $propertyNames = array(), $depth = 0) {
        ...
        if(($pathProps = $this->getPathProperties($path, $propertyNames, $parentNode)) !== false) {
            $returnPropertyList[] = $pathProps;
        }
        if ($depth==1 && $parentNode instanceof ICollection) {
            foreach($this->tree->getChildren($path) as $childNode) {
                if(($pathProps = $this->getPathProperties($path . '/' . $childNode->getName(), $propertyNames, $childNode)) !== false) {
                    $returnPropertyList[] = $pathProps;
                }
            }
        }
    ...
    public function getPathProperties($path, $propertyNames = array(), $node = null)
    {
        ...
        $result = $this->broadcastEvent('beforeGetProperties', array($path, $node, &$propertyNames, &$newProperties));
        ...
        if (count($propertyNames) > 0) {
            ...
            if ($node instanceof IProperties) {
                ...
                // rest.
                foreach ($propertyNames as $k => $currentPropertyName) {
